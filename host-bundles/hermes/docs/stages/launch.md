...
Pre-GATE-1 worker model: Wave 0 (discovery) and Wave 1 (PM) spawn BEFORE the team model table
  ...
  hermes default (e.g. Fable 5), which may be a model the user forbids. Instead, run
  the configured provider/model list once, pick the cheapest AVAILABLE model NOT in roster.json
  `forbidden_models` (the user may name a different temporary model), and pass it explicitly
  at spawn (`model=` in delegate_task tool). Log the choice in `.pmos/log.md`. This is temporary:
