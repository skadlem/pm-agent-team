...
1. Read `~/.hermes/pmos-template-root` to get `TPL` (on Windows:
   `type %USERPROFILE%\.hermes\pmos-template-root`).
...
10. GATE 1: present roster + model selection. If `~/.hermes/pmos-team-defaults.json` exists, propose
    that role -> model table as-is (user's saved preference; verify its models still appear in
    the configured provider/model list). Otherwise compute via the configured provider/model list ->
