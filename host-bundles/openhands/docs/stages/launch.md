...
Pre-GATE-1 worker model: Wave 0 (discovery) and Wave 1 (PM) spawn BEFORE the team model table
  ...
  openhands default (e.g. Fable 5), which may be a model the user forbids. Instead, run
  your LiteLLM provider's model list once, pick the cheapest AVAILABLE model NOT in roster.json
  `forbidden_models` (the user may name a different temporary model), and pass it explicitly
  at spawn (OH_MODEL on the openhands runner tool). Log the choice in `.pmos/log.md`. This is temporary:
