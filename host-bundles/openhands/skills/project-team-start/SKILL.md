...
1. Read the file `~/.openhands/pmos-template-root` (e.g. `type %USERPROFILE%\.openhands\pmos-template-root`
...
5. GATE 1 (STOP and ask the user): present the proposed roster and scope summary, AND the model
   selection. If `~/.openhands/pmos-team-defaults.json` exists, propose it as the role -> model table
   (user's saved preference; verify its models are still in your LiteLLM provider's model list). Otherwise compute
   it LIVE:
   ...
   c. The user can OK all, change a model/effort, or remove a role. Record the approved map in
      ...
      an OpenHands one-shot spawn (tools/openhands_run.py). Adjust roster on request.
