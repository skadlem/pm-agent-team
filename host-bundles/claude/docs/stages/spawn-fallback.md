...
Spawn via the `claude -p` headless session with a clear `label` like "pm", "architect", "backend-1". Use one
...
3. Spawn a FRESH worker for that task, passing the next model explicitly (`--model` on the claude CLI
